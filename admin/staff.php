...
<body>
    <div class="addroomsection">
        <form action="" method="POST">
            ...
            <button type="submit" class="btn btn-success" name="addstaff">Add Staff</button> <!-- Changed button text from "Add Room" to "Add Staff" -->
        </form>
        ...
        if (isset($_POST['addstaff'])) {
            ...
            // Corrected column name from 'work' to 'role' in the INSERT query
            $sql = "INSERT INTO staff(name, role) VALUES ('$staffname', '$staffwork')";
            $result = mysqli_query($conn, $sql);

            if ($result) {
                header("Location: staff.php");
            } else {
                echo "Error: " . mysqli_error($conn); // Added error handling for debugging
            }
        }
        ?>
    </div>
    ...
    <?php
        ...
        $re = mysqli_query($conn, $sql); // Added semicolon
        ?>
        <?php
        while ($row = mysqli_fetch_array($re)) {
                // Corrected column name from 'work' to 'role' for display
